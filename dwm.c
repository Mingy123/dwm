...
static void togglelockfs(const Arg *arg);
static void togglefakefs(const Arg *arg);
static void maketiled(const Arg *arg);
...
clientmessage(XEvent *e)
{
    ...
    if (cme->message_type == netatom[NetWMState]) {
        ...
        || cme->data.l[2] == netatom[NetWMFullscreen]) {
            int full;
            if (fakefullscreen) full = cme->data.l[0] == 1 || cme->data.l[0] == 2;
            else full = cme->data.l[0] == 1 || (cme->data.l[0] == 2 && !c->isfullscreen);
...
movemouse(const Arg *arg)
{
    ...
    if (!(c = selmon->sel))
        return;
    if (!fakefullscreen && c->isfullscreen)
        return;
    restack(selmon);
    ocx = c->x;
    ocy = c->y;
...
showhide(Client *c)
{
    ...
    if (ISVISIBLE(c)) {
        ...
        int thing = !c->mon->lt[c->mon->sellt]->arrange || c->isfloating;
        if (!fakefullscreen) thing = thing && !c->isfullscreen;
        if (thing)
...
togglefloating(const Arg *arg)
{
    if (!selmon->sel)
        return;
    if (!fakefullscreen && selmon->sel->isfullscreen)
        return;
...
void togglelockfs (const Arg *arg) {
    if (lockfullscreen) lockfullscreen = 0;
    else lockfullscreen = 1;
}
